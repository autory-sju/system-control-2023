...
const int direction_output_pin = 3; 
...
void loop() {
  ...
  if (Serial.available() > 0) {                                  // 시리얼 버퍼에 데이터가 있는지 확인
    serialInputVal = (int)Serial.readStringUntil('\n').toInt();  // 시리얼 입력 값을 정수로 읽어옴
    ...
    Serial.print(motorOutputVal);
    Serial.println("/255 -> output Value");
